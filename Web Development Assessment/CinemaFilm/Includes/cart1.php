...
if (!empty($_SESSION['cart']))
{
  ...
  $q = "SELECT * FROM movies WHERE movie_id IN (";
  ...
  while ($row = mysqli_fetch_array ($r, MYSQLI_ASSOC))
  {
    ...
    # Display the row/s:
    echo "<h5>Movie Title: </h5>  {$row['title']}  </td>
		  <tr><td><h5>Show Time: </h5> {$row['showing2']}</td></tr> 
		  <tr><td><h5>Tickets: 
	<input type=\"text\" size=\"3\" name=\" qty[{$row['movie_id']}]\" value=\" {$_SESSION['cart'][$row['movie_id']]['quantity']}\">
